...
long sizeOfFileRequested(int fd) {
    char filename[MAXLINE], buf[MAXLINE], method[MAXLINE], uri[MAXLINE], version[MAXLINE], cgiargs[MAXLINE];
    struct stat st;
...
char* requestedFileName(int fd) {
    char  buf[MAXLINE], method[MAXLINE], uri[MAXLINE], version[MAXLINE], cgiargs[MAXLINE];

    recv(fd, buf, sizeof(buf), MSG_PEEK);
    sscanf(buf, "%s %s %s\n", method, uri, version);
    requestParseURI(uri, reqfilename, cgiargs);
    return reqfilename;
}

int requestedFileNameLength(int fd) {
    char  buf[MAXLINE], method[MAXLINE], uri[MAXLINE], version[MAXLINE], cgiargs[MAXLINE],filename[MAXLINE];
